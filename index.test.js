...
describe("render", () => {
  ...
  it("should throw an error for invalid data", () => {
    expect(() => render("{{name}}", null)).toThrow(
      "Invalid data: must be an object"
    );
    expect(() => render("{{name}}", "string")).toThrow(
      "Invalid data: must be an object"
    );
  });
...
describe("render_template", () => {
  it("should render a template from a file", () => {
    ...
    expect(fs.readFileSync).toHaveBeenCalledWith(
      path.join(templatePath, basename + ".hbs"),
      "utf8"
    );
  });
  ...
  it("should throw an error if template file is not found", () => {
    ...
    expect(() => {
      ...
      `Template file not found: ${path.resolve(
        options.template_path,
        basename + options.template_extension
      )}`
    );
  });
  ...
  it("should throw an error if basename is not string", () => {
    ...
    expect(() => render_template(basename, data, options)).toThrowError(
      "Invalid basename: must be a non-empty string"
    );
  });
  ...
  it("should throw an error if read file throws error", () => {
    const basename = "test_template";
    const data = { name: "World" };
    const templatePath = path.join(__dirname, "__mocks__"); // Path to your mock templates
    const options = { template_path: templatePath, template_extension: ".hbs" };
    ...
    expect(async () =>
      render_template(basename, data, options)
    ).rejects.toThrowError("Error reading template file: some error");
  });
  ...
  it("should use default options when none are provided", () => {
    ...
    const result = render_template(basename, data);
